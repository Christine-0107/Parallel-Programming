...
void block_gauss_neon_omp(float** matrix,int my_rank, int num_proc,int n)
{
    int block_size = n / num_proc;
    int remain = n % num_proc;
...
void block_run(int version,float** matrix,int n)
{
	...
	if (version == 0) {
		...
		f = &block_gauss_neon_omp;
		inform="block+neon+omp:";
	}
	struct timeval start;
    struct timeval end;
    unsigned long diff;
...
void recycle_gauss_neon_omp(float** matrix,int my_rank, int num_proc,int n)
{
    #pragma omp parallel num_threads(NUM_THREADS)
    for (int k = 0; k < n; k++) {
        #pragma omp single
...
void recycle_pipeline_neon_omp(float** matrix,int my_rank, int num_proc,int n)
{
    int pre_rank = (my_rank - 1 + num_proc) % num_proc;
	int nex_rank = (my_rank + 1) % num_proc;
	#pragma omp parallel num_threads(NUM_THREADS)
...
void recycle_run(int version,float** matrix,int n)
{
	...
	if (version == 0) {
		...
		f = &recycle_gauss_neon_omp;
		inform="recycle+neon+omp:";
	}
	else if (version == 2) {
		f = &recycle_pipeline;
		inform="pipeline:";
		...
		f = &recycle_pipeline_neon_omp;
		inform="pipeline+neon+omp:";
	}
	struct timeval start;
    struct timeval end;
    unsigned long diff;
